...
def handle_request(path):
    ...
    if session_id not in session_containers:
        ...
        container = client.containers.run(
            "my-flet-app", detach=True, network=network_name
        )
...
def cleanup():
    ...
    for session_id, ip_address in list(session_containers.items()):
        container = client.containers.list(
            filters={"network": network_name, "ancestor": "my-flet-app"}
        )
        for cont in container:
            if (
                cont.attrs["NetworkSettings"]["Networks"][network_name]["IPAddress"]
                == ip_address
            ):
                cont.stop()
                cont.remove()
