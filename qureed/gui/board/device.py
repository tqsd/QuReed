...
class Device(BaseDeviceComponent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        base_height = 50
        max_ports = max(len(self.ports_in.ports), len(self.ports_out.ports))

        if max_ports >= 4:
            base_height = max_ports * 15

        super().__init__(height=base_height, width=width, *args, **kwargs)
        # Compute the full path for the image
        if getattr(sys, "frozen", False):
            # If the application is frozen (packaged by PyInstaller)
            base_path = sys._MEIPASS
            base_path = os.path.join(base_path, "qureed", "gui", "assets")
        else:
