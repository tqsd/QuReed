...
class ConstantClock(GenericClockDevice):
    """
    Emits periodic `TriggerSignal`s at a constant rate defined
    by the `frequency` property.

    This clock device starts ticking at `start_time` (if current time is
    earlier) and continues to emit signals until the simulation time reaches
    `end_time`. The interval between ticks is computed as the inverse of
    `frequency`.
    ...
    - If `frequency <= 0`, no ticks will be emitted (can be validated
      externally).
    """
    ...
    def proc(self):
        ...
        at the beginning of the simulation and then waits for `1/frequency`
        time units before the next click.
        ...
        while True:
            ...
            yield self.sim_env.timeout(1 / frequency)
