...
from quasi.signals import (
    GenericBoolSignal, GenericQuantumSignal)

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.append(os.path.dirname(SCRIPT_DIR))

from time_synchronization.custom_components.entangled_photon_source import EntangledPhotonSource
...
signals["1"] = connect(
    GenericBoolSignal, st, "trigger",
    source, "trigger"
)
signals["2"] = connect(
    GenericQuantumSignal, source, "output",
    fiber1, "input"
)
signals["3"] = connect(
    GenericQuantumSignal, fiber1, "output",
    fltr, "input"
)
